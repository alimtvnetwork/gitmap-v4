...
//
// This mirrors the marker rules used by completion/internal/gencommands so a
// passing test guarantees the dispatch table and shell completion list are
...
// to this test file so the test runs the same way from `go test ./...` as it
// does from inside an IDE.
